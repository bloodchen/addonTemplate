...
export class User extends BaseService {
  ...
  async createUser({ email, password, frm = 0, info = {}, status = 1 }) {
    ...
    // 设定默认初始信息
    const defaultInfo = { dataShards: 500, computePower: 30 };
    const finalInfoObj = { ...(info || {}), ...defaultInfo };
    ...
    const newUser = await db.insert('users', {
      ...
      pass: hash,
      frm,
      info: JSON.stringify(finalInfoObj),
      status
    });
  ...
  async handleLoginSuccessful_fromCommonAPI({ OTT, ...rest }) {
    ...
    if (type === 'maxthon') {
      ...
      const mxvip = (rest?.vip?.level && rest?.vip?.state === 'enabled') ? rest?.vip?.level : 0
      await this.ensureUser({ email, frm: 2, info: { avatar: avatar_url, mxvip } })
    }
    if (type === 'email') {
  ...
  async regEndpoints(app) {
    ...
    app.get('/user/info', async (req, res) => {
      try {
        ...
        return user ? user : { err: 'user-not-found' };
      } catch (error) {
